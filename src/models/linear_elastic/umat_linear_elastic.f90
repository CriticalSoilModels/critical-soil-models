!! UMAT wrapper for the linear elastic model.
!!
!! ### PROPS layout
!!
...
!! | 2     | ν      | Poisson's ratio     | —     |
!!
!! No STATEV are used — linear elastic has no internal state.
...
contains
   ...
   subroutine umat_linear(STRESS, STATEV, DDSDDE,          &
      ...
      !! Standard Abaqus UMAT entry point for the linear elastic model.
      !!
      !! Builds a `linear_elastic_model_t` from `PROPS`, inflates the solver
      !! vectors to internal 6-component form, integrates, then deflates back.
      !! Because the yield function always returns F < 0 the integrator exits
      !! immediately after the elastic predictor.
