!! Array utility functions.

module mod_array_helper
   use mod_csm_kinds, only: wp

   implicit none
   private
   public :: reorder_real_array, all_unique_integer

contains

   pure function all_unique_integer(arr) result(is_unique)
      !! Return `.true.` if every element of `arr` is distinct.
      integer, intent(in) :: arr(:)  !! Integer array to check
      logical :: is_unique
      integer :: i, j

      is_unique = .true.

      do i = 1, size(arr) - 1
         do j = i + 1, size(arr)
            if (arr(i) == arr(j)) then
               is_unique = .false.
               return
            end if
         end do
      end do
   end function all_unique_integer

   function reorder_real_array(input_arr, new_order) result(output_arr)
      !! Reorder a real array by index permutation.
      !!
      !! `new_order` must be a permutation of `[1 … n]` where `n = size(input_arr)`.
      !! Returns unallocated if sizes mismatch or `new_order` is not a valid permutation.
      real(kind=wp), intent(in)  :: input_arr(:)   !! Source array
      integer,       intent(in)  :: new_order(:)   !! Permutation index array
      real(kind=wp), allocatable :: output_arr(:)

      integer :: i, n
      logical :: is_valid_order

      n = size(input_arr)

      if (size(input_arr) /= size(new_order)) then
         print *, "Error: The length of the input array and the new order isn't the same"
         print *, "The length of the input arr is: ", size(input_arr)
         print *, "The length of the new order arr is: ", size(new_order)
         return
      end if

      is_valid_order = all(new_order >= 1 .and. new_order <= n) .and. all_unique_integer(new_order)
      if (.not. is_valid_order) then
         print *, "Error: Invalid reordering array"
         return
      end if

      allocate(output_arr(n))

      do i = 1, n
         output_arr(i) = input_arr(new_order(i))
      end do

   end function reorder_real_array

end module mod_array_helper
