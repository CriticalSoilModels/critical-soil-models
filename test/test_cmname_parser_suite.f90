...
contains
   ...
   subroutine collect_cmname_parser_suite(testsuite)
      ...
      testsuite = [ &
         ...
         new_unittest("material_name: cpa suffix",             test_material_cpa),         &
         ...
         new_unittest("integrator_name: cpa suffix",           test_integrator_cpa),       &
   ...
   subroutine test_material_cpa(error)
      type(error_type), allocatable, intent(out) :: error
      character(len=80) :: cmname
      cmname = "MCSS_INTEG_CPA"
      call check(error, trim(material_name(cmname)), "MCSS", &
                 more="material_name: should strip _INTEG_CPA")
   end subroutine test_material_cpa
   ...
   subroutine test_integrator_cpa(error)
      type(error_type), allocatable, intent(out) :: error
      character(len=80) :: cmname
      cmname = "MCSS_INTEG_CPA"
      call check(error, trim(integrator_name(cmname)), "cpa", &
                 more="integrator_name: CPA suffix should give lowercase cpa")
   end subroutine test_integrator_cpa
